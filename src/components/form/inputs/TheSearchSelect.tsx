...
export function TheSearchSelect<T,>({
 default_value,
  ...
  const { data, loading, error } = useHandRolledQuery<Awaited<ReturnType<typeof fetcherFn>>>({
  ...
  return (
    <div className="flex h-full w-full cursor-pointer flex-wrap items-center justify-start ">
      <div className="lex w-full flex-col items-center gap-2">
        <TheTextInput field_name={"search"} />
        {loading&&<div className="flex w-full items-center justify-center  text-sm text-info">
          loading...
        </div>}
        {error&&<div className="flex w-full items-center justify-center  text-sm text-error">
          {error.message}
        </div>}
